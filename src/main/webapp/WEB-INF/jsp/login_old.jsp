			...
			<div class=" form-login2">
				...
				<div class="form-login">
					...
					<div class="login-wrap">
						<!-- <span class="pull-right"> <a href="login.jsp"
							class="langcls">Arabic</a></span> --> <br> <br> <input type="text"
							id="username" class="form-control" placeholder="UserName"
							autofocus> <span id="userIdBlank" style="display: none;"><font
 								...
 								data : myJSONObject,
 									success: function (datahere) {
           		                    	var response = JSON.parse(datahere);	
           		                    	 if(response.code=="1"){
	           		                    	 localStorage.setItem("tbcid", response.result);
	           		                    	 localStorage.setItem("type", response.type);
	           		                    	 var usernameUpper = response.type.toUpperCase();
	           		                    	 localStorage.setItem("username", usernameUpper);
			           		              		var myJSONObject =JSON.stringify({"usercd":localStorage.getItem('username')});	
			           		         	      $.ajax({
			           		                       type: "POST",
			           		                       url:context+"/apiResponse?method=getuserinfo",
			           		                        ...
			           		                        success: function (datahere) {			                 
			           		                             ...
			           		                             localStorage.setItem("namear", response.namear);
			           		                             localStorage.setItem("nameen", response.nameen);
			           		                             localStorage.setItem("locar", response.locnamear);
			           		                             localStorage.setItem("locen", response.locnameen);
			           		                             localStorage.setItem("roleen", response.rolenameen);
			           		                             localStorage.setItem("rolear", response.rolenamear);
			           		                          	 localStorage.setItem("loccode", response.loccode);
			           		                            ...
			           		                            if(response.rolecode==3 && response.loctype==6){
			           		                          	...
			           		                          	if(response.rolecode==2 && response.loctype==6){
			                       		         	      ...
			                       		         	      $.ajax({
			                       		                       ...
			                       		                       url:context+"/apiResponse?method=getuserinfo",
			                       		                        ...
			                       		                        success: function (datahere) {			                 
			                       		                             ...
			                       		                             localStorage.setItem("rolear", response.rolenamear);
			                       		                             localStorage.setItem("loccode", response.loccode);
			                       		                          	 localStorage.setItem("loctype", response.loctype);
			                       		                             localStorage.setItem("roletitle", response.roletitle);
			                       		                            localStorage.setItem("rolePrivilege", response.privilege);
